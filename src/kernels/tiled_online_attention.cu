...
__global__ void tiled_online_attention_kernel(
    ...
    float scale)
{
    ...
    float O_reg[DPL];
    #pragma unroll
    for (int j = 0; j < DPL; ++j) O_reg[j] = 0.f;
...
void launch_tiled_online_attention_bhsd(const float* Q, const float* K, const float* V,
                                        float* O,
                                        ...
                                        cudaStream_t stream)
{
    ...
    tiled_online_attention_kernel<Br, Bc, Dt><<<grid, block, 0, stream>>>(
        Q, K, V, O, N, scale);
}
