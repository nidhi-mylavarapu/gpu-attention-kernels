...
// Fill with N(0, 0.02) — not meaningful for correctness of softmax shape,
// but keeps values from blowing up the exp.
...
struct BenchResult {
    ...
    size_t peak_bytes;
    double max_abs_err;   // vs CPU, or NaN if skipped
};
                                 ...
                                 bool check_correctness)
{
    ...
    size_t mem_before = gpu_mem_used_bytes();
...
int main(int argc, char** argv) {
    ...
    for (int S : seq_lens) {
        ...
        bool check = (S <= 512);   // CPU ref is slow beyond this
        BenchResult r = benchmark_one(handle, cfg, warmup, iters, check);
        printf("%d,%.3f,%.3f,%.3f,%.1f,%.1f,%s\n",
               r.seq_len, r.mean_ms, r.min_ms, r.max_ms,
